...
class PublicHTTPRequestHandler(HTTPRequestHandlerRTP):
    ...
    def renderIndexPage(self):
        ...
        try:
            ...
            # Generate the api endpoint used to access the Mikrotik router
            routerCtrlApiUrl = f"http://{parent.listenAddr}:{parent.listenPort}/api/sshcmd" \
                               f"?deviceAddress={controllerDefinitions['mikrotikAddress']}&" \
                               f"username={controllerDefinitions['mikrotikUsername']}&commandString="
            # Insert the URL into the html template by finding/replacing the string $apiUrlSendCmdToMikrotik
            htmlFile = htmlFile.replace("<apiUrlSendCmdToMikrotik>", routerCtrlApiUrl)
            # Generate the Selector buttons
            # buttonJS = """<button onclick="sendCommandToMikrotik(':put%20%22$[/system%20clock%20get%20time]%22')">Send Mikrotik Command</button>"""
            infoText = "<!-- Generated by studiocontroller.py:PublicHTTPRequestHandler.renderIndexPage() -->\n"
            buttonsHtml = infoText + "\n".join([f"""<button onclick="sendCmdToMikrotik('system script run {button["target_script"]}')">{button["label"]}</button>"""
...
def main():
    ...
    sharedObjects["controllerDefinitions"] = {
        "mikrotikAddress": "192.168.3.2",
        "mikrotikUsername": "kabulctrl",
